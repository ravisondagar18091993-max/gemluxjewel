...
const templatePath =
  process.argv[2] ||
  path.join(process.env.USERPROFILE || '', 'Downloads', 'product_template (1).csv');
...
const PRODUCT = {
  ...
  description:
    'Demo product with unique prices for Silver, Platinum, and each gold karat/color (White, Yellow, Rose).',
...
function priceFor(metal, diamond) {
  return METAL_BASE[metal] + DIAMOND_ADD[diamond];
}
...
const rows = [joinCsvLine(templateHeader)];
let isFirst = true;
...
for (const metal of METALS) {
  for (const diamond of DIAMONDS) {
    for (const ring of RINGS) {
      ...
      setCol(row, 'Weight value (grams)', '10');
      setCol(row, 'Weight unit for display', 'g');
